...
module.exports = class playCommand extends Command {
 ...
 async execute(msg, args, discord, client) {
  ...
  if(!voiceChannel) return msg.channel.send('Ses Kanalına Bağlı Olmanız gerekiyor. Ses kanalına Bağlandıktan sonra tekrar deneyiniz.');
  ...
  async function play(guild, song) {
   ...
   const dispatcher = await serverQueue.connection.play(stream)
    ...
    const embedSong = new discord.MessageEmbed()
     ...
     .setDescription(`Süre: **${durationHours}:${durationMinutes}:${durationSeconds}**`)
     .setURL(song.url)
     .setFooter(song.publish);

    return msg.channel.send(`Şimdi çalıyor:`, {embed: embedSong})
  ...
  async function handleVideo(video, playlist) {
   ...
   if (serverQueue) {
    ...
    return msg.channel.send('Şarkı sıraya eklendi!: ', { embed: embedQueue });
    ...
    try {
     ...
     msg.channel.send('Oynatma hatası oluştu.')
    }
  ...
  if(!args[0]) return msg.channel.send('Bir youtube bağlantısı eklemelisiniz.')
  let video;
  ...
  if (ytdl.validateURL(args[0])) {
   ...
   try {
    ...
    if (!videos.length) return msg.channel.send('Arama sonucu bulunamadı, Youtube bağlantısını göndermeyi deneyin')
    let index = 0
    ...
    try {
     ...
     return msg.channel.send('Arama seçeneği iptal edildi.')
    ...
    return msg.channel.send('Arama sonuçlarında bir hata oluştu.')
